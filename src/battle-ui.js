...
engine.subscribe(next => { state = next; render(); });
...
function battlefieldHtml(){
  const active=unit(state.activeUnitId); const enemies=state.units.filter(u=>u.team==='enemy');
...
function allyHudHtml(){
  const allies=state.units.filter(u=>u.team==='ally'); const activeCandidate=unit(state.activeUnitId); const active=activeCandidate.team==='ally'?activeCandidate:allies[0]; const others=allies.filter(u=>u.id!==active.id);
...
function skillDockHtml(){
  const active=unit(state.activeUnitId); const order=['basic','skill','forkAlt','ultimate'];
...
root.addEventListener('click',e=>{
  const target=e.target.closest('[data-target]'); if(target){ engine.selectTarget(target.dataset.target); return; }
  const skill=e.target.closest('[data-skill]'); if(skill){ engine.castSkill(skill.dataset.skill); return; }
